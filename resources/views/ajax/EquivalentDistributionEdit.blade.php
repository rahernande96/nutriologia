                                ...
                                @if(in_array($food_group->id, $params['food_group']))
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="row">
                                                    <div class="col-sm-12 pt-2">
                                                    <input type="number" class="form-control" step="0.1" name="field[{{ $food_group->id }}][equivalent_food_group]" @foreach($details as $detail) @if($detail['food_group_id'] == $food_group->id) value="{{$detail->fields['equivalent_food_group']}}" @endif @endforeach>
                                                    </div>
                                                    <div class="col-sm-12 pt-2">
                                                        <div class="btn-group">
                                            ...
                                            @foreach($params['days'] as $day)
                                                @foreach($params['food_time'] as $ft)
                                                <td>
                                                    <input class="form-control" name="field[{{ $food_group->id }}][{{ $day }}][{{ $ft }}][equivalent]" type="number" min="0" step="0.1" @foreach($details as $detail) @if($detail['food_group_id'] == $food_group->id && isset($detail->fields[$day][$ft]['equivalent'])) value="{{ $detail->fields[$day][$ft]['equivalent'] }}" @endif @endforeach>
                                                </td>
                                                @endforeach
                                                <td>
                                                    <input type="number" name = "field[{{ $food_group->id }}][{{ $day }}][missing_equivalent]" class="form-control" min="0" step="0.1" @foreach($details as $detail) @if($detail['food_group_id'] == $food_group->id && isset($detail->fields[$day]['missing_equivalent'])) value="{{ $detail->fields[$day]['missing_equivalent'] }}" @endif @endforeach>
                                                </td>
                                            @endforeach
                                        </tr>
                                @endif
